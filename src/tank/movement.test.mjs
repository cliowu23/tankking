...
const C = { acceleration: 4.5, drag: 20, brakeDecel: 28 };
const near = (a, b, eps = 1e-6) => Math.abs(a - b) <= eps;

// Forward accel from rest
...
// Time to top speed ~3.5s (16 / 4.5)
let s = 0; for (let t = 0; t < 3.56; t += 1 / 60) s = Math.min(16, stepThrottle(s, true, false, 1 / 60, C));
assert.ok(s >= 15.9, `spool-up reaches top (~${s.toFixed(2)})`);
